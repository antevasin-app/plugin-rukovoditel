...
class email_rules
{
    ...
    function prepare_send_to( $rules )
    {
        ...
        switch ( $rules['action_type'] )
        {
            ...
            case 'schedule_send_to_assigned_email':
                ...
                break;
            case 'send_to_emails':
                if ( is_array( $rules['send_to_emails'] ) )
                {
                    $send_to = array_unique( $rules['send_to_emails'] );
                }
                break;
        }
    ...
    static function send_by_id( $entity_id, $item_id, $rule_id, $emails = false, $description = false )
    {
        if ( $emails )
        {
            $sql = "
                SELECT r.* 
                FROM app_ext_email_rules r, app_entities e  
                WHERE 
                    r.entities_id=e.id AND r.id='{$rule_id}' AND 
                    length( subject ) >0 AND 
                    length( description ) >0 AND 
                    ( length( r.send_to_users ) >0 OR 
                    length( r.send_to_assigned_users ) >0 OR 
                    length( r.send_to_email ) >0 OR 
                    length( r.send_to_assigned_email ) >0 OR 
                    action_type='schedule_send_by_visibility_rules') AND 
                    r.is_active=1
            ";
            $sql = "
                SELECT r.* 
                ...
                WHERE 
                    ...
                    r.is_active=1
            ";
            $rules_query = db_query( $sql );
            // print_rr($rules_query);
        }
        else
        {
        ...
        {
            if ( $emails ) { 
                $rules['action_type'] = 'send_to_emails'; 
    ...
    function send_by_id_msg( $rules, $email_field_id = false )
    {
        if ( !$rules )
        {
            return false;
